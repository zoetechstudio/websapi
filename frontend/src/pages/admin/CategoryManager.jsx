...
const CategoryManager = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Yakin ingin menghapus kategori ini? (Sapi dengan kategori ini mungkin akan error)')) return;
    try {
      const token = localStorage.getItem('adminToken');
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="p-4 text-xs font-black text-gray-500 uppercase tracking-wider">Ikon</th>
                <th className="p-4 text-xs font-black text-gray-500 uppercase tracking-wider">Nama Kategori</th>
                <th className="p-4 text-xs font-black text-gray-500 uppercase tracking-wider">Slug</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-xl font-black text-gray-900">{editingId ? 'Edit Kategori' : 'Tambah Kategori Baru'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Nama Kategori</label>
                <input 
                  type="text" required
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="ekonomis / premium / dll"
                />
              </div>
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Ikon (Emoji)</label>
                <input 
                  type="text"
                  className="w-full px-4 py-2.5 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-primary-500 font-medium"
                  value={formData.icon}
